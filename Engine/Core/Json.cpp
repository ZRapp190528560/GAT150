...
namespace AZ {
	bool AZ::json::Load(const std::string& filename, rapidjson::Document& document){
		bool success = false;
		std::ifstream stream(filename);
		ASSERT(stream.good(), "Error loading JSON file: " + filename);
		if (stream.is_open())
		{
	...
	bool AZ::json::Get(const rapidjson::Value& value, const std::string& name, int& data){
